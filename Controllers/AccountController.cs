            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await emailService.SendEmailAsync(model.Email, "Подтверждение вашего аккаунта",
                        $"Подтвердите вашу регистрацию, перейдя по ссылке" + $" <a href='(callback)'>link</a>");
                    return Content("Для завершения регистрации проверьте свою почту");
                }
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if(user != null)
                {
                    ...
                    {
                        ModelState.AddModelError(string.Empty, "Вы не подтвердили регистрацию.");
                        return View(model);
                    }          
                }
